...
include('session.php');
include("../config.php");
if(@$_GET['action'] == 'promo'){
  $sqlpromo = "update promotion set bID = '$_GET[id]' where promoID = '1'";
  mysqli_query($conn,$sqlpromo) or die(mysqli_error());
}
?>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
<title>จัดการหนังสือแนะนำ</title>
      ...
      *********************************************************************************************************************************************************** -->
      ...
      <section id="main-content">
          <section class="wrapper site-min-height">
          	<h3><i class="fa fa-angle-right"></i> จัดการโปรโมชั่นหน้าร้าน</h3>
          	<?php
                $psql = "select * from promotion where promoID = '1'";
                $prs = mysqli_query($conn,$psql);
                $pdata = mysqli_fetch_array($prs);

              ?>
              <form action="" method="post" class="form-inline">
                  <table border="0" class="table table-striped">
                      <tr>
                        <td width="20%">ชื่อโปรโมชั่น</td>
                        <td><input type="text" name="promoName" class="form-control" value="<?=$pdata['promoName'];?>"></td>
                      </tr>
                          <tr>
                        <td width="20%">คำอธิบาย</td>
                        <td><TextArea name="promoDesc" class="form-control" ><?=$pdata['promoDesc'];?></TextArea></td>
                      </tr>
                      <tr>
                        <td width="20%">รหัสหนังสือ</td>
                        <td><input type="text" name="bID" class="form-control" value="<?=$pdata['bID'];?>"></td>
                      </tr>
                      <tr>
                      <td></td>
                          <td><input type="submit" name="submit" value="บันทึก" class="btn btn-success"></td>
                      </tr>
              ...
              <?php
                ...
                $proname = $_POST['promoName'];
                $prodesc = $_POST['promoDesc'];
                $bID = $_POST['bID'];
